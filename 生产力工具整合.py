...
class SimpleToolLauncher:
    def __init__(self, launcher_obj=None):
        # 保存launcher对象的引用，用于手动更新
    ...
    def create_main_window(self):
        ...
        bottom_frame.pack(fill=tk.X, padx=30, pady=10)
        
        # 更新按钮
        update_btn = tk.Button(bottom_frame, text="🔄 更新主界面",
                              command=self.manual_update_interface,
    ...
    def manual_update_interface(self):
        """手动更新主界面功能"""
        # 确认对话框
        result = messagebox.askyesno("更新确认", 
                                   "确定要更新主界面吗？\n\n更新过程中程序将重启。")
        if not result:
            return
        
        # 显示更新进度窗口
        update_window = self.show_update_progress()
        
        def do_update():
            try:
                # 更新状态
                self.root.after(0, lambda: self.update_status_in_window(update_window, "正在连接..."))
                time.sleep(0.5)
                
                # 检查是否有launcher对象可用
                if hasattr(self, 'launcher') or 'launcher' in globals():
                    launcher_obj = getattr(self, 'launcher', globals().get('launcher'))
                    if launcher_obj:
                        self.root.after(0, lambda: self.update_status_in_window(update_window, "正在下载最新版本..."))
                        
                        # 强制更新主界面代码
                        main_code = launcher_obj.download_main_interface(force_update=True)
                        
                        if main_code:
                            self.root.after(0, lambda: self.update_status_in_window(update_window, "更新成功，正在重启..."))
                            time.sleep(1)
                            
                            # 关闭更新窗口
                            self.root.after(0, lambda: update_window.destroy())
                            
                            # 重启界面
                            self.root.after(100, lambda: self.restart_interface(main_code))
                        else:
                            self.root.after(0, lambda: self.update_failed(update_window, "无法获取更新"))
                    else:
                        self.root.after(0, lambda: self.update_failed(update_window, "更新服务不可用"))
                else:
                    # 如果没有launcher对象，尝试直接从GitHub API获取
                    self.root.after(0, lambda: self.update_status_in_window(update_window, "正在获取..."))
                    main_code = self.download_from_github_api()
                    ...
                    if main_code:
                        ...
                        self.root.after(0, lambda: self.update_failed(update_window, "网络连接失败"))
                        
            except Exception as e:
                self.root.after(0, lambda: self.update_failed(update_window, f"更新失败: {str(e)}"))
        
        # 在后台线程执行更新
        threading.Thread(target=do_update, daemon=True).start()
    
    def download_from_github_api(self):
        """直接从GitHub API下载主界面代码"""
        try:
            # GitHub仓库信息
            owner = 'jwwl520'
            repo = 'Productivity-tool-integration'
            file_path = '生产力工具整合.py'
            
            url = f"https://api.github.com/repos/{owner}/{repo}/contents/{file_path}"
            
            headers = {
                'Accept': 'application/vnd.github.v3+json',
                'User-Agent': 'Tool-Launcher-Update'
            }
    ...
    def show_update_progress(self):
        ...
        # 内容框架
        frame = tk.Frame(update_window, bg='#f0f0f0', padx=20, pady=20)
        frame.pack(fill=tk.BOTH, expand=True)
        
        # 图标和标题
        icon_label = tk.Label(frame, text="🔄", font=("Microsoft YaHei UI", 24), bg='#f0f0f0')
        icon_label.pack(pady=(0, 10))
        
        title_label = tk.Label(frame, text="正在更新主界面", 
                              font=("Microsoft YaHei UI", 12, "bold"),
                              bg='#f0f0f0', fg='#2c3e50')
        title_label.pack(pady=(0, 10))
        
        # 状态标签
        status_label = tk.Label(frame, text="准备更新...",
                               font=("Microsoft YaHei UI", 9),
                               bg='#f0f0f0', fg='#666666')
        ...
        # 存储组件引用到窗口对象
        update_window.status_label = status_label
        update_window.progress = progress
        
        return update_window
    
    def update_status_in_window(self, window, status):
        """更新窗口中的状态文本"""
        if hasattr(window, 'status_label'):
            window.status_label.config(text=status)
    
    def update_failed(self, window, error_msg):
        """处理更新失败"""
        try:
    ...
    def restart_interface(self, new_code):
        """重启界面使用新代码"""
        try:
            # 保存当前状态
            current_processes = self.tool_processes.copy()
            ...
            # 如果有launcher对象，传递给新代码
            if hasattr(self, 'launcher'):
                globals_dict['launcher'] = self.launcher
            elif 'launcher' in globals():
                globals_dict['launcher'] = globals()['launcher']
            
            exec(new_code, globals_dict)
            
        except Exception as e:
            messagebox.showerror("重启失败", f"重启界面失败: {str(e)}")
...
def main():
    ...
    try:
        # 检查是否有传入的launcher对象
        launcher_obj = globals().get('launcher', None)
        launcher = SimpleToolLauncher(launcher_obj)
        launcher.run()
        ...
        # 静默处理异常，避免弹出控制台
        try:
            messagebox.showerror("程序错误", f"程序异常退出: {e}")
        except:
            pass
